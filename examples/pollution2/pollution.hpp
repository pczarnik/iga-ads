...
const int iterations = 3000; // 30000
...
private:
    ...
    double S = 1;
    return S * sin(10 * 3.14159265358979 * x) * sin(10 * 3.14159265358979 * y) * std::max(0., sin(3.14159265358979 * t / 10));    
    }
        ...
        for (auto e : elements()) {
            ...
            for (auto q : quad_points()) {
                ...
                for (auto a : dofs_on_element(e)) {
                    ...
                    double h = e2h(e[1]);
                    auto p = point(e, q);
                    double val = u.val * v.val - steps.dt * gradient_prod + steps.dt * (dTy(h) + cannon(p[0], p[1], t)) * u.dy * v.val + steps.dt * f(h) * v.val;
                    rhs(a[0], a[1]) += val * w * J;
                }
            }
        }
